...
const SearchBar = () => {
    ...
    if (throttledQuery !== "") {
      ...
      axios(config)
        .then(function (response) {
          ...
          receivedQuery.current = response.data.results;
        })
  ...
  const handleKeyMoves = (e) => {
    if (e.keyCode === 40) {
      if (currentSel >= receivedQuery.current.length) {
        selectCurrent(1);
      } else {
      ...
      if (currentSel === 1) {
        selectCurrent(receivedQuery.current.length);
      } else {
        selectCurrent((prev) => prev - 1);
      }
      ...
      let crn = receivedQuery.current[currentSel - 1];
      let url = crn.url;
  ...
  return (
    <>
      ...
      {!isloading && receivedQuery.current.length > 0 && (
        <>
          <AllResult selected={currentSel}>
            {receivedQuery.current.map(
              ({ name, birth_year, gender, url }, index) => (
                <SuggestionBlock
                  key={index}
                  url={url}
